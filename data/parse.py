...
class CustomDataset(Dataset):
    def __init__(self, data_turgan_yolak, ds_nomi, tfs=None, data_type=None, rasm_fayllari=[".png", ".jpg", ".jpeg", ".bmp"]):
    ...
    def get_files(self): 
        ...
        elif self.ds_nomi in ["facial_expression"]: self.im_paths = [path for im_file in self.rasm_fayllari for path in glob(f"{self.root}/{self.data_type}/*/*{im_file}")]
        elif self.ds_nomi in ["pokemon"]: self.im_paths = [path for im_file in self.rasm_fayllari for path in glob(f"{self.root}/{self.data_type}/*/*{im_file}")]
